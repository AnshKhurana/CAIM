        ...
        vector<int> > node_to_feat, unordered_map<int, vector<pair<int, int> > > feat_to_edges, vector<int> F, int K, int I, unordered_map <int, double> &in_degrees, edge_prob &P, int mode=0)
{
    ...
    {
        cf.push_back(f);
        changed = increase_probabilities_no_transform(G, B, Q, node_to_feat, cf, feat_to_edges[f], P, in_degrees);
        spread = calculate_MC_spread(G, P, S, I);
        cout<<"f = "<<f<<" "<<"MC spread: "<<spread<<endl;
        // decrease_probabilities(changed, P);
        // increase_probabilities_no_transform(G, B, Q, node_to_feat, cf, feat_to_edges[max_feature], P, in_degrees);
    }
}
...
int main(int argc, char const *argv[])
{
    ...
    double final_spread;
    vector <int> result_feature_set{99, 81, 36, 12, 66, 39, 4, 13, 23, 73, 24, 97, 80, 57, 20, 25, 71, 52, 16, 60, 49, 37, 17, 58, 48, 47, 91, 42, 18, 19, 64, 85, 51, 53, 65, 94, 40, 54, 83, 21, 2, 9, 44, 56, 93, 38, 98, 84, 82, 22};
