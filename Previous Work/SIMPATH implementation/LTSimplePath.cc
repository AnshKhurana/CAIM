...
void LTSimplePath::openOutputFiles() {
	...
	string algorithm = "SimPath";
	cout << "problem : " << problem << endl;
	string filename = outdir + "/" + m + "_" + algorithm + "_" + opt->getValue("topl") + "_" + opt->getValue("cutoff") + ".txt";
	outFile.open (filename.c_str()); // file for storing seeds info
	...
	// for storing avg #hops.
	if (hopsOutFile.is_open())
		hopsOutFile.close();
	
	string hopFilename = outdir + "/" + m + "_" + algorithm + "_" + opt->getValue("cutoff") + "_Hops.txt";
	hopsOutFile.open(hopFilename.c_str());
	if (hopsOutFile.is_open() == false) {
		cout << "Can't open file " << hopFilename << " for writing" << endl;
		exit(1);
	}
